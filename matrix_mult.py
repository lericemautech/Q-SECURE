...
def matrix_vector_mult(matrix_1: np.ndarray, matrix_2: np.ndarray, index: int) -> tuple:
    """
    Multiply 2 matrices
    ...
    Returns:
        tuple: Result of multiplying Matrix #1 and Matrix #2, and its index
    """
...
def processing(m1_results: list, m2_results: list) -> np.ndarray:
    ...
    with ProcessPoolExecutor(N_WORKERS) as manager:
        for i in range(len(m1_results)):
            to_be_executed = manager.submit(matrix_vector_mult, m1_results[i], m2_results[i % VERTICAL_PARTITIONS], num)
            processes.append(to_be_executed)
            num += 1
