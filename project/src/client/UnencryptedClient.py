...
class UnencryptedClient():
    def __init__(self, matrix_a: ndarray, matrix_b: ndarray, length: int = LENGTH, matrix_b_width: int = MATRIX_B_WIDTH):
        ...
        CLIENT_LOGGER.info("Starting Unencrypted Client...\n")
        ...
        while not self._partitions.empty():
            try:
                with socket() as sock:
                    ...
                    CLIENT_LOGGER.info(f"Unencrypted Client connected to Server at {server_address} in {timing(connection_timer, start)} seconds\n")
                    ...
                    CLIENT_LOGGER.info(f"Unencrypted Client connected, sent, received, and unpacked data from Server at {server_address} in {timing(end, start)} seconds\n")
                ...
                CLIENT_LOGGER.info(f"Unencrypted Client worked for {timing(end_work, start_work)} seconds\n")
...
if __name__ == "__main__":    
    ...
    # Create Unencrypted Client to multiply matrices
    unencrypted_client = UnencryptedClient(matrix_a, matrix_b)
